 ...
 */
static const char* BITCOIN_PID_FILENAME = "infinitycoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("infinitycoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/infinitycoin-project/infinitycoin>";
    const std::string URL_WEBSITE = "<https://infinitycoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("infinitycoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if infinitycoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if infinitycoin is started while in a temporary directory.\n",
